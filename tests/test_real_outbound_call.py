"""
Test Real Outbound Call with UAE VM LiveKit Credentials

This test demonstrates the complete outbound call flow using:
- UAE VM LiveKit server (91.74.244.94:7880)
- Agent ID 33
- Phone number +971545335200 (has livekit_config UUID in database)
...
async def test_real_outbound_call():
    """
    Test making a real outbound call using UAE VM credentials.
    
    Configuration:
    - From: +971545335200 (UAE number with livekit_config in DB)
    - To: Test number (UAE format recommended)
    - Agent: 33
    - LiveKit: UAE VM (91.74.244.94:7880)
    - Trunk: ST_svHE4RdTc7Ds
    """
    print("="*80)
    print("REAL OUTBOUND CALL TEST - UAE VM LiveKit")
    print("="*80)
    
    # Configuration
    from_number = "0545335200"  # UAE number with livekit_config UUID
    to_number = "0506341191"  # Local UAE format (will be formatted by routing)
    agent_id = 33
    voice_id = "default"  # Will resolve from agent's default voice
    initiated_by = None  # No user context for this test
    added_context = "This is a test call from the UAE VM LiveKit server."
    lead_name = "Test Lead"
    ...
    # Step 3: Resolve tenant_id from agent
    print(f"\n4. Resolving Tenant ID...")
    tenant_id = await call_service._agent_storage.get_agent_tenant_id(agent_id)
    if tenant_id:
        print(f"   ✓ Tenant ID: {tenant_id[:8]}...")
    else:
        print(f"   ✗ Could not resolve tenant_id from agent {agent_id}")
        return
    ...
    print(f"   ✓ Using UAE VM server: {livekit_creds.url}")
    print(f"   ✓ Trunk ID: {livekit_creds.trunk_id}")
    print(f"   ✓ Worker Name: {livekit_creds.worker_name}")
    print(f"   ✓ Agent ID: {agent_id}")
    print(f"   ✓ Call Log ID: {result.call_log_id}")
...
if __name__ == "__main__":
    ...
    # Run test
    asyncio.run(test_real_outbound_call())
